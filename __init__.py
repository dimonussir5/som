...
def man_dist_pbc(m: np.ndarray, vector: np.ndarray, shape: tuple = (10, 10)) -> np.ndarray:
    ...
    :param shape: shape of the full simom
...
class simom(object):
    ...
    def __init__(self, x: int, y: int, alpha_start: float = 0.6, sigma_start: float = None, seed: int = None):
        """Initialize the simom object with a given map size and training conditions
    ...
    def initialize(self, data: np.ndarray, how: str = "pca"):
        ...
        :type how: str
        :return: initialized map in :py:attr:`simom.map`
        """
    ...
    def cycle(self, vector: np.ndarray, verbose: bool = True):
        """Perform one iteration in adapting the simom towards a chosen data point
    ...
    def fit(
        ...
        """Train the simom on the given data for several iterations
        ...
        if save_e:  # save the error to history every "interval" epochs
            for i in range(epochs):
                ...
                if i % interval == 0:
                    self.history.append(self.simom_error(data))
            self.error = self.simom_error(data)
        else:
            for i in range(epochs):
                self.cycle(data[indx[i]], verbose=verbose)

    def transform(self, data: np.ndarray) -> np.ndarray:
        """Transform data in to the simom space

        :param data: data to be transformed
        :type data: np.ndarray
        :return: transformed data in the simom space
        :rtype: np.ndarray
        """
    ...
    def distance_map(self, metric: str = "euclidean"):
        ...
        :type metric: str
        :return: normalized sum of distances for every neuron to its neighbors, stored in :py:attr:`simom.distmap`
        """
    ...
    def winner_map(self, data: np.ndarray) -> np.ndarray:
        """Get the number of times, a certain neuron in the trained simom is the winner for the given data.
    ...
    def _one_winner_neuron(self, data: np.ndarray, q: Queue):
        ...
        :return: winner neuron cooridnates for every datapoint (see :py:method:`simom.winner_neurons`)
        """
    ...
    def _one_error(self, data: np.ndarray, q: Queue):
        ...
        :param data: data matrix to calculate simom error for
        :type data: np.ndarray
        :param q: queue
        :type q: multiprocessing.Queue
        :return: list of simom errors (see :py:method:`simom.simom_error`)
        """
        errs = []
        for d in data:
            [x, y] = self.winner(d)
    ...
    def simom_error(self, data: np.ndarray) -> float:
    ...
    def plot_point_map(
        ...
        """Visualize the simom with all data as points around the neurons

        :param data: data to visualize with the simom
        :type data: np.ndarray
    ...
    def plot_density_map(
        ...
        """Visualize the data density in different areas of the simom.

        :param data: data to visualize the simom density (number of times a neuron was winner)
        :type data: np.ndarray
        :param colormap: colormap to use, select from matplolib sequential colormaps
        :type colormap: str
    ...
    def plot_error_history(self, color: str = "orange", filename: Union[str, None] = None):
        ...
        if not len(self.history):
            raise LookupError("No error history was found! Is the simom already trained?")
        fig, ax = plt.subplots()
        ax.plot(range(0, self.epoch, self.interval), self.history, "-o", c=color)
        ax.set_title("simom Error History", fontweight="bold")
    ...
    def save(self, filename: str):
        """Save the simom instance to a pickle file.
    ...
    def load(self, filename: str):
        """Load a simom instance from a pickle file.
