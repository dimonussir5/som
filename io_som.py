...
def main(in_file, out_file, x, y, epochs, ref=None, test=False, verbose=0):
    ...
    simom = simom(x, y)
    if ref == 'IRCI':
        simom = simom.load('/simom.pkl')
        embedding = simom.winner_neurons(df.values)
    else:
        simom.fit(df.values, epochs, verbose=verbose)
        embedding = simom.winner_neurons(df.values)
        if ref == 'Create':
            simom.save(basedir + '/simom.pkl')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-x', '--x', dest='x', action='store', type=int, default=10,
                        help='Size of the simom in x-coordinate')
    parser.add_argument('-y', '--y', dest='y', action='store', type=int, default=10,
                        help='Size of the simom in y-coordinate')
    ...
    if args.test:
        ...
        plt.title('simom test plot')
        plt.savefig('simom_test.png')
    else:
